...
urlpatterns = patterns('',
    ...
    (r'^\d{4}/\d{2}/\d{2}/(?P<postid>\d+)/$','pylogs.blog.views.post'),
    (r'^\d{4}/\d{2}/\d{2}/(?P<postname>\S+)/$','pylogs.blog.views.post'),
    ...
    (r'^(?P<year>\d{4})/(?P<month>(\d{2})?)/?(?P<date>(\d{2})?)/?$','pylogs.blog.views.dateposts'),
